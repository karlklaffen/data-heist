...
# Ensure Logs/Regression directory exists
if not os.path.exists(os.path.join('Logs', 'Regression')):
    os.makedirs(os.path.join('Logs', 'Regression'))
...
print("\nData Analysis Log:")
write_log(data_log, os.path.join('Logs', 'Regression', 'data_analysis_no_scaler'))
comparison_log.update(data_log)
write_log(comparison_log, os.path.join('Logs', 'Regression', 'model_comparison_no_scaler'))

print("\nModel Logs:")
for model_name, model_log in model_logs.items():
    filename = f"model_{model_name.replace(' ', '_').replace('(', '').replace(')', '').lower()}"
    write_log(model_log, os.path.join('Logs', 'Regression', filename))
